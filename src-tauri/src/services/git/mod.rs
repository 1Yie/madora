...
pub async fn status(root_path: PathBuf) -> Result<GitStatus, String> {
	run_repo_task(root_path, |root_path| status::read_status_or_empty(&root_path)).await
}
...
pub async fn push(
	...
	run_repo_task(root_path, move |root_path| {
		remote::push(&root_path, &remote_name, branch_name.as_deref(), auth_config.as_ref())
	})
	.await
}

pub async fn stage_file(root_path: PathBuf, path: PathBuf) -> Result<GitStatus, String> {
	run_repo_task(root_path, move |root_path| branch::stage_file(&root_path, &path)).await
}

pub async fn unstage_file(root_path: PathBuf, path: PathBuf) -> Result<GitStatus, String> {
	run_repo_task(root_path, move |root_path| branch::unstage_file(&root_path, &path)).await
}

pub async fn restore_file(root_path: PathBuf, path: PathBuf) -> Result<GitStatus, String> {
	run_repo_task(root_path, move |root_path| branch::restore_file(&root_path, &path)).await
}

pub async fn read_log(root_path: PathBuf, limit: usize) -> Result<Vec<GitLogEntry>, String> {
	run_repo_task(root_path, move |root_path| history::read_log(&root_path, limit)).await
}

pub async fn undo_last_commit(root_path: PathBuf) -> Result<GitSyncResult, String> {
	run_repo_task(root_path, move |root_path| history::undo_last_commit(&root_path)).await
}
...
pub async fn list_branches(root_path: PathBuf) -> Result<Vec<GitBranchInfo>, String> {
	run_repo_task(root_path, move |root_path| branch::list_branches(&root_path)).await
}

pub async fn create_branch(root_path: PathBuf, branch_name: String) -> Result<GitStatus, String> {
	run_repo_task(root_path, move |root_path| branch::create_branch(&root_path, &branch_name)).await
}

pub async fn switch_branch(root_path: PathBuf, branch_name: String) -> Result<GitStatus, String> {
	run_repo_task(root_path, move |root_path| branch::switch_branch(&root_path, &branch_name)).await
}
