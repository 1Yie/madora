...
fn merge_parent_commit(repo: &Repository) -> GitResult<Option<git2::Commit<'_>>> {
	...
	let merge_head = repo.find_reference("MERGE_HEAD").map_err(|_| {
		GitServiceError::message("检测到合并状态，但缺少 MERGE_HEAD 引用")
	})?;
	let merge_parent_id = merge_head.target().ok_or_else(|| {
		GitServiceError::message("MERGE_HEAD 没有指向有效提交")
	})?;
	repo.find_commit(merge_parent_id).map(Some).map_err(Into::into)
}
