...
function computeNewVersion(oldVersion, spec) {
  if (spec === "major" || spec === "minor" || spec === "patch") {
    ...
    if (!parsed) throw new Error(`Current version '${oldVersion}' is not semver`);
...
if (fs.existsSync(cargoTomlPath)) {
  try {
    ...
    if (pkgIndex === -1) {
      console.warn(`Warning: [package] section not found in ${cargoTomlPath}, skipping`);
    } else {
      let replaced = false;
      ...
      if (!replaced) {
        console.warn(`Warning: version key not found under [package] in ${cargoTomlPath}, skipping`);
      } else {
