...
export const ModelSelectorTrigger = ({
	...
	return (
		<Pressable
			className={`w-[200px] h-[40px] bg-primary justify-between
				${className || ''}`}
			onPress={handlePress}
			{...props}
		>
...
export const ModelSelectorInput = ({
	...
	<TextInput
		className={`h-12 px-4 border-b border-border text-base text-foreground
...
export const ModelSelectorItem = ({
	...
	return (
		<Pressable
			className={`flex-row items-center h-4 w-4 px-4 py-3
				${isSelected ? 'bg-accent' : 'active:bg-muted'} ${className || ''}`}
			{...props}
		>
			{children}
		</Pressable>
...
export const ModelSelectorName = ({
	...
	<Text
		className={`flex-1 text-left text-base ml-3 text-foreground
			${className || ''}`}
	>
		{children}
	</Text>
