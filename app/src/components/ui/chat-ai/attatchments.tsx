			...
			<Box
				className={` group relative
					${variant === 'grid' ? 'w-24 h-24 overflow-hidden rounded-lg' : ''} ${
						variant === 'inline'
							? `flex h-8 items-center gap-1.5 rounded-md border border-border
			...
			return (
				<Image
					...
					className={`size-full object-cover
						${variant !== 'grid' ? 'rounded' : 'rounded-xl'}`}
	...
	return (
		<Box
			className={` flex shrink-0 items-center justify-center overflow-hidden
				...
				${variant === 'list' ? 'size-12 rounded bg-muted' : ''} ${className} `}
			{...props}
		>
			{renderContent()}
		</Box>
	...
	return (
		<Button
			...
			className={` ${
				variant === 'grid'
					? isWeb
						? `absolute top-2 right-2 size-6 rounded-full bg-background/80
							opacity-0 group-hover:opacity-100`
						: `absolute top-3 right-3 h-8 w-8 items-center flex justify-center
							rounded-full bg-muted`
					: ''
				} ${variant === 'inline' ? 'size-5 ml-auto' : ''}
				${variant === 'list' ? 'size-8 shrink-0 ml-auto' : ''} ${className} `}
			variant="ghost"
			size="sm"
			{...props}
		>
