...
import { useMadoraSync } from '@/features/madora-sync/provider';
import { QrScanner } from '@/features/madora-sync/qr-scanner';
import {
	ActionButton,
	MetricTile,
	Panel,
		...
		<View style={{ flex: 1, backgroundColor: '#fbfcff' }}>
			...
			>
				...
				<Panel className="gap-3">
					...
					<View className="flex-row gap-3">
						<MetricTile
							label={t('syncSettings.metrics.docs')}
							value={String(storageStats.documents)}
						/>
						<MetricTile
							label={t('syncSettings.metrics.pending')}
							tone={storageStats.pendingUpdates > 0 ? 'warm' : 'default'}
							value={String(storageStats.pendingUpdates)}
						/>
					</View>
					<View className="flex-row gap-3">
						<MetricTile
