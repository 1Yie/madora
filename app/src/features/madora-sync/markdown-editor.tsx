...
import { useSetMarkdownToolbar } from './markdown-toolbar-context';
import { MarkdownPreview } from './markdown-preview';
import type {
	MarkdownCompletionControl,
	MarkdownToolbarIcon,
} from './markdown-toolbar-context';
...
export function MarkdownEditor({
	...
	useEffect(() => {
		...
		if (apiRef.current)
			injectEditorStyle(apiRef.current, fontSize, contentTopPadding);
	}, [contentTopPadding, fontSize]);
	...
	const handleInitialized = useCallback(
		(api: WebViewAPI) => {
			...
			injectEditorStyle(api, fontSize, contentTopPadding);
...
function injectEditorStyle(
    ...
    (function() {
      ...
      style.textContent = ${JSON.stringify(getEditorCss(fontSize, contentTopPadding))};
...
function getEditorCss(fontSize: number, contentTopPadding: number) {
    ...
    .cm-content {
      caret-color: #2563eb;
      min-height: 100%;
      padding: ${contentPaddingTop}px 18px 14px !important;
