...
const CUSTOM_SWATCH = "conic-gradient(red, yellow, lime, cyan, blue, magenta, red)";
...
export function AppearanceSettings() {
  const { theme, setTheme, accent, accentMode, setAccent, setAccentMode } = useTheme();
  const { customCss, setCustomCss } = useProseTheme();

  const sorted = [...PRESETS].sort((a, b) => {
    const aRgb = hexToRgb(a[0]);
    const bRgb = hexToRgb(b[0]);
    return rgbToHsl(aRgb.r, aRgb.g, aRgb.b).h - rgbToHsl(bRgb.r, bRgb.g, bRgb.b).h;
  });
  ...
  const isCustom =
    accentMode === "custom" && accent !== null && !sorted.some(([hex]) => hex === accent);

  return (
    <div className="space-y-4">
      <SettingsSectionCard description="选择应用的主题模式，跟随系统、浅色或深色" title="主题模式">
      ...
      <SettingsSectionCard description="配置应用的主题强调色" title="主题颜色">
        <div className="grid grid-cols-3 gap-2.5 md:grid-cols-4 mb-4">
          ...
          >
            <div className="h-9 w-9 shrink-0 rounded-lg" style={{ background: SYSTEM_SWATCH }} />
          ...
          >
            <div className="h-9 w-9 shrink-0 rounded-lg" style={{ background: DEFAULT_SWATCH }} />
          ...
          >
            ...
            <div className="flex h-full items-center gap-3 px-4 py-3">
              ...
              >
                ...
                {isCustom && (
                  <span className="absolute bottom-0.5 right-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-white/90">
              ...
              <div>
                ...
                {isCustom && (
                  <div className="font-mono text-[11px] text-muted-foreground">{accent}</div>
                )}
              </div>
            </div>
          </label>
          ...
          {sorted.map(([hex, label]) => (
            ...
            >
              <div className="flex h-full w-full flex-col">
                <div className="relative h-10 w-full" style={{ background: hex }}>
                  {selectedPreset === hex && (
                    <span className="absolute bottom-1 right-1 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-white/90">
                      <CheckIcon className="h-3 w-3 text-gray-800" />
                    </span>
                  )}
                </div>
      ...
      <SettingsSectionCard description="自定义 Markdown 预览的 CSS 样式" title="自定义 CSS">
        <div className="[&_textarea]:min-h-60 [&_textarea]:font-mono [&_textarea]:text-sm">
          <Textarea
        ...
        <div className="pt-2">
          <p className="text-xs text-muted-foreground">
            选择器应以 <code>.prose-custom</code> 为前缀，以避免与应用其他部分的样式冲突。
          </p>
        </div>
      </SettingsSectionCard>
    </div>
