...
function getContrastTextColor(hex: string) {
  ...
  const darkContrast = getContrastRatio(backgroundLuminance, getRelativeLuminance({ r: 15, g: 23, b: 42 }));
...
function getInitialAccentMode(): AccentMode {
  ...
  if (storedMode === "default" || storedMode === "system" || storedMode === "custom") {
    return storedMode;
  }

  return normalizeHexColor(window.localStorage.getItem(ACCENT_STORAGE_KEY)) ? "custom" : "system";
}
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [accentMode, setAccentModeState] = useState<AccentMode>(getInitialAccentMode);
  const [accent, setAccentState] = useState<string | null>(getInitialAccent);
  const effectivePrefersDark = theme === "system" ? (systemPrefersDark ?? prefersDark) : prefersDark;
  ...
  useEffect(() => {
    ...
    const accentOverlay = getOverlayColor(normalizedAccent, resolvedTheme === "dark" ? 0.22 : 0.14);
    const sidebarAccentOverlay = getOverlayColor(normalizedAccent, resolvedTheme === "dark" ? 0.26 : 0.16);
    ...
    root.style.setProperty("--sidebar-accent", sidebarAccentOverlay ?? normalizedAccent);
      ...
      value={{
        ...
        toggleTheme: () => {
          setTheme((currentTheme) =>
            resolveTheme(currentTheme, effectivePrefersDark) === "dark" ? "light" : "dark",
          );
        },
        systemAccent,
        accent,
