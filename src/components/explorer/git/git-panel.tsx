...
const workbenchSections = [
  ...
  { id: "history" as GitWorkbenchTab, label: "历史", description: "提交记录", icon: History },
  { id: "remote" as GitWorkbenchTab, label: "远端", description: "远端同步配置", icon: Settings2 },
  { id: "ssh" as GitWorkbenchTab, label: "SSH", description: "认证凭据设置", icon: KeyRound },
];
...
function getSummary(status: GitStatus | null): string {
  ...
  if (status.totalChangedCount === 0) {
    return parts.length > 0 ? `工作区干净 · ${parts.join(" · ")}` : "工作区干净";
  }
...
export function GitPanel({
  ...
  const loadBranches = async () => {
    ...
    try {
      ...
      showErrorToast("获取分支列表失败", error instanceof Error ? error.message : String(error));
    } finally {
  ...
  const createBranch = async () => {
    ...
    try {
      ...
      showErrorToast("创建分支失败", error instanceof Error ? error.message : String(error));
    } finally {
  ...
  const pickSshPrivateKeyFile = async () => {
    ...
    try {
      const selectedPath = await invoke<string | null>("git_pick_ssh_private_key_file");
      ...
      showErrorToast("选择文件失败", error instanceof Error ? error.message : String(error));
    } finally {
      setActionBusy(false);
    }
  };
    ...
    <>
      <div className="flex min-w-0 flex-1 items-center justify-between gap-2 px-2">
        {status?.hasRepository ? (
          <Popover onOpenChange={handleBranchPopoverOpen} open={branchPopoverOpen}>
            <Tooltip>
              <TooltipTrigger
                  ...
                  {branches.length === 0 ? (
                    ...
                    branches.map((branch) => (
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center rounded-full gap-2 px-3 py-2 text-left text-sm transition-colors hover:bg-sidebar-accent disabled:opacity-50",
                          branch.isHead
              ...
              {activeTab === "history" && (
                <GitTabHistory
                  ...
                  onRevertRequest={(commitId, summary) =>
                    setPendingHistoryAction({ type: "revert-commit", commitId, summary })
                  }
